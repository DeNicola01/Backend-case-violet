...
export function Button({
  ...
  const getVariantClasses = () => {
    ...
    const variants = {
      ...
      ghost: 'border border-transparent bg-transparent text-gray-700 hover:bg-gray-100 focus:ring-gray-500'
    };
  ...
  const renderIcon = () => {
    if (loading) {
      return (
        <svg 
          className={`animate-spin ${getIconSize()} ${iconPosition === 'left' ? 'mr-2' : 'ml-2'}`} 
          fill="none" 
          viewBox="0 0 24 24"
        >
          <circle 
    ...
    if (icon) {
      return (
        <span className={`${getIconSize()} ${iconPosition === 'left' ? 'mr-2' : 'ml-2'}`}>
          {icon}
        </span>
      );
    }
